...
static void samsung_pinmux_setup(struct pinctrl_dev *pctldev, struct pinctrl_gpio_range *range,
					unsigned offset, bool enable)
{
	...
	drvdata = pinctrl_dev_get_drvdata(pctldev);
	func = &drvdata->pmx_functions[range->base]; //TODO: ijh check how to convert from selector to range
	grp = &drvdata->pin_groups[offset];
...
static int samsung_pinmux_enable(struct pinctrl_dev *pctldev, struct pinctrl_gpio_range *range,
					unsigned offset)
{
	samsung_pinmux_setup(pctldev, range, offset, true);
	return 0;
}
...
static void samsung_pinmux_disable(struct pinctrl_dev *pctldev,
				   ...
				   unsigned offset)
{
	samsung_pinmux_setup(pctldev, range, offset, false);
}
...
static const struct pinmux_ops samsung_pinmux_ops = {
	...
	.gpio_request_enable	= samsung_pinmux_enable,
	.gpio_disable_free		= samsung_pinmux_disable,
				     ...
				     unsigned num_configs)
{
	...
	for (cnt = 0; cnt < drvdata->pin_groups[selector].num_pins; cnt++)
